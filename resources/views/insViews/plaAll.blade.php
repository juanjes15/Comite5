<x-app-layout>
    <x-slot name="header">
        <h2 class="leading-tight">
            {{ __('Planes de mejoramiento activos') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12 text-black font-medium">
            ...
            <div class="bg-gray2 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <div class="flex items-center justify-between m-4">
                        <form action="{{ route('insViews.plaAll') }}" method="get" class="flex-grow ml-4">
                            ...
                            @forelse ($plans as $plan)
                                <tr class="border-b">
                                    <td class="px-6 py-4 truncate max-w-xs">
                                        {{ $plan->comite->solicitud->aprendizs[0]->ficha->fic_codigo }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ $plan->pla_estado }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ $plan->pla_fechaInicio }}
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <x-link2 title="Editar" href="{{ route('insViews.plaUpd', $plan) }}">
                            ...
                            @empty
                                <tr class="bg-white border-b">
                                    <td colspan="5" class="px-6 py-4 whitespace-nowrap">
                                        {{ __('No se encontraron planes activos') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <br>
        {!! $plans->appends(['q' => request()->input('q')])->links() !!}
    </div>
</x-app-layout>
